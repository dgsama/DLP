...
%left INC DEC
%left '*' '/' '%'
%left ADD_ASSIG SUB_ASSIG PROD_ASSIG DIV_ASSIG
...
statement:		RETURN exp ';' 						{ $$ = new Return(scanner.getLine(), scanner.getColumn(), (Expression)$2); }
				...
				| ternario 							{ $$ = $1;}
				| exp ADD_ASSIG exp ';'				{ $$ = new Assignment(scanner.getLine(), scanner.getColumn(), (Expression)$1, (Expression)new ArithmeticOperation(scanner.getLine(), scanner.getColumn(), (Expression)$1, (Expression)$3, "+")); }
				| exp SUB_ASSIG exp ';'				{ $$ = new Assignment(scanner.getLine(), scanner.getColumn(), (Expression)$1, (Expression)new ArithmeticOperation(scanner.getLine(), scanner.getColumn(), (Expression)$1, (Expression)$3, "-")); }
				| exp PROD_ASSIG exp ';'			{ $$ = new Assignment(scanner.getLine(), scanner.getColumn(), (Expression)$1, (Expression)new ArithmeticOperation(scanner.getLine(), scanner.getColumn(), (Expression)$1, (Expression)$3, "*")); }
				| exp DIV_ASSIG exp ';'				{ $$ = new Assignment(scanner.getLine(), scanner.getColumn(), (Expression)$1, (Expression)new ArithmeticOperation(scanner.getLine(), scanner.getColumn(), (Expression)$1, (Expression)$3, "/")); }
				| switch 							{$$ = $1;}
				| do_while							{ $$ = $1;}
...
switch:			SWITCH '(' exp ')' '{' cases default '}'	{ $$ = parseSwitchToIfElse(scanner.getLine(), scanner.getColumn(),(Expression)$3, (List<Statement>)$6, (List<Statement>)$7) ;}
				;

cases:			cases case							{ ((List<Statement>)$$).add((Statement)$2); $$ = $1; }
				|case								{ List <Statement> aux = new ArrayList<Statement>(); aux.add((Statement)$1); $$ = aux; }
				;

case:			CASE exp ':' statements BREAK ';'	{ $$ = new IfElse(scanner.getLine(), scanner.getColumn(), (Expression)$2, (List<Statement>)$4, new ArrayList());}
				;

default: 		DEFAULT ':' statements BREAK ';'    { $$ = $3; }
				;
...
exp:	exp '+' exp                               		{ $$ = new ArithmeticOperation(scanner.getLine(), scanner.getColumn(), (Expression)$1, (Expression)$3, "+"); }
		...
		| exp '=' exp 									{ $$ = new AssignmentExpr(scanner.getLine(), scanner.getColumn(), (Expression)$1, (Expression)$3); }
